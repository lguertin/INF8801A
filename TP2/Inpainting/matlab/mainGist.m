...
% 
% for n = 0:5
%     filtre = desc.getGabor(deg2rad(n * pas), lda, sigmaX, sigmaY);
%     figure; imagesc(filtre); colormap('gray');
%     for k = 1:4
%         filtre = desc.getGabor(deg2rad(n * pas), lda / (k * 2), sigmaX / (k * 2), sigmaY / (k * 2));
%         figure; imagesc(filtre); colormap('gray'); title(['Filtre ' num2str(n)]);
%     end
% end

% affichage des valeurs du descripteur
figure; image(64*desc.display()); title('descripteur GIST');
