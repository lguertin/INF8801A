% Pour cette question, vous devez implÈmenter la fonction descGist.m qui prend en entrÈe une image
% (‡ mettre en niveaux de gris) et qui retourne la rÈponse de líimage ‡ 30 filtres de Gabor (6 angles
% et 5 Èchelles, c.f. figure 1). La valeur absolue de la rÈponse de chaque filtre est moyennÈe sur les
% rÈgions de líimage correspondant ‡ une grille de 4x4 (attention ‡ la maniËre dont vous moyennez ;
% níutilisez pas de filtre Nearest). Nous obtenons donc un total de 480 valeurs. Ces valeurs peuvent
% Ítre affichÈes sous forme de graphique avec la fonction descGist.display() qui vous est donnÈe (c.f.
% figure 2).


classdef descGist
   %DESCGIST Descripteur d'images GIST
   % d√©di√© aux sc√®nes ext√©rieures
    
   % variables Statiques
   properties (Constant)
       
       nbOri = 6; % nombre d'orientation de filtres
       nbScales = 5; % nombre d'√©chelles de filtres
       gridSize = 4; % taille de la grille de filtres
   
       filterSize = 64; % taille des kernels
       imageSize = 128; % plus petit c√¥t√© de l'image
       filters = descGist.getGabors(); % kernels des filtres de Gabor
   end
   
   properties
       
       % r√©ponses des divers filtres :
       % [ grilleY, grilleX, √©chelle, orientation ]
       values;
   end
   ...
   methods (Static)
       ...
       function filters = getGabors()
        ...
        sigmaY = 32; % gaussienne perpendiculaire √† l'onde (px)
        for n = 0:5
